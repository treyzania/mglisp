...
pub fn mgi_lambda(args: &Vec<Sexp>, env: &mut Env) -> Result<Rc<LispValue>, EvalError> {
    ...
    Ok(Rc::new(LispValue::Func(Box::new(LispFunction::Lambda(Rc::new(args[1].clone()), env.clone(), names)))))
...
pub fn mgi_if(args: &Vec<Sexp>, env: &mut Env) -> Result<Rc<LispValue>, EvalError> {
    ...
    match cond.as_ref() {
        &LispValue::Boolean(true) => eval(&args[1], env),
        &LispValue::Boolean(false) => eval(&args[2], env),
